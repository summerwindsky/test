...
public class XmlUtil {
    ...
    private static String VALUE_PATH = "/@value";

    public static Document getDocBystr(String xml) {
        Document doc_new = null;
        try {
            doc_new = DocumentHelper.parseText(xml);
        } catch (DocumentException e) {
            logger.error("文书转换dom对象失败", e);
        }
        return doc_new;
    }
    ...
    public static String getSingleValue(Element elem, String xpath) {
        ...
        Node node = elem.selectSingleNode(xpath + VALUE_PATH);
        if (null == node) {
            return null;
        }
    ...
    public static String getValue(Element elem, String path) {
        Node node = elem.selectSingleNode(path + VALUE_PATH);
        if (null == node) {
            return null;
        }
        return node.getStringValue();
    ...
    public static String getSingleValue(Document doc, String xpath) {
        ...
        Node node = doc.selectSingleNode(xpath + VALUE_PATH);
        if (null == node) {
            return null;
        }
        return node.getStringValue();
    ...
    public static String getValues(Element node, String xpath) {
        ...
        List<Attribute> els = node.selectNodes(xpath + VALUE_PATH);
    ...
    public static List<String> getAttributeListValues(Element elem, String xpath) {
        ...
        @SuppressWarnings("unchecked")
        List<Attribute> lst = elem.selectNodes(xpath + VALUE_PATH);
        if (!CollectionUtils.isEmpty(lst)) {
            for (Attribute el : lst) {
                values.add(el.getText());
            }
     ...
     * @return
     */
    public static Attribute getSingleNode(Document doc, String xpath) {
        if (doc == null || StringUtils.isBlank(xpath)) {
            return null;
        }
        return (Attribute) doc.selectSingleNode(xpath);
    }
